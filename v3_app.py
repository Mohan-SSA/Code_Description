...
# ---------- Core helpers ----------
def llm_chat(system_prompt: str, user_prompt: str, temperature: float = 0.1) -> str:
    """
    ...
    sorted_providers = sorted(
        [(name, cfg) for name, cfg in providers.items() if cfg.get("enabled", True)],
    ...
    for provider_name, cfg in sorted_providers:
        ...
        try:
            ...
            error_msg = f"{provider_name} request failed: {e}"
            logger.error(error_msg)
            errors.append(error_msg)
            if hasattr(e, 'response') and e.response is not None and e.response.status_code == 429:
                continue
        except (KeyError, IndexError, json.JSONDecodeError) as e:
            error_msg = f"{provider_name} response parsing failed: {e}"
            logger.error(error_msg)
            errors.append(error_msg)
            continue
